                       ...
                       refillRate = .025)             
{
  ...
  return( data.frame(
    ...
    microbeK = 900 * necVol, #900 cells per ul # https://bsapubs.onlinelibrary.wiley.com/doi/pdfdirect/10.1002/ajb2.1834 says higher but our data suggest otherwise?
    necVol = necVol,
    maxNectar = maxNectar,
...
SugarValFun  <- function(bee,flower){
  #this is linear increase in preference up to the preference parameter then a 
  #steep drop to zero above that measure
...
SugarValFun(bee=data.frame(sugarPref=12), flower=data.frame(sugar=20))
  ...
  decide <- function(bee, flower){
              # bee senses a flower, decides whether to visit based on its threshold
                ...
                # second half of if statement makes a small chance (8%) that bees visit flowers that are below their threshold
                  if(bee[bee$time==currentTime-1,"thresholdF1"] < flower[flower$time==currentTime, "signal"]|runif(1,0,100)<=8){
...
#  probabilities of flowers in the environment
  ...
  beePopParms <- rep(seq(from=1,to=5),4)
  
  #
  simulations  <- mclapply(X=beePopParms,FUN=function(x){simulation(nbees =x)}, mc.cores = 11)
  simulations2 <- mclapply(X=beePopParms,FUN=function(x){simulation(nbees =x)}, mc.cores = 11)
  ...
  is.data.frame(bees)
  for(i in 1:length(beePopParms)){
    df <- do.call(rbind,simulations2[[i]]$bees)
    df$beePop <- beePopParms[i]
    df$simnum <-i+length(beePopParms)
    bees<-rbind(bees,df)
  }
  ...
  for(i in 2:length(beePopParms)){
    df <- do.call(rbind,simulations2[[i]]$flowers)
    df$beePop <- beePopParms[i]
    df$simnum<-i + length(beePopParms)
    flowers<-rbind(flowers,df)
  }
  ...
  SumFlowers <- flowers %>% group_by(time,simnum,beePop) %>%
    summarise(
      nectarVol = mean(necVol),
      microbePop = mean(microbesP1))
  ggplot(SumFlowers, aes(x=time,y=nectarVol,group=interaction(beePop,simnum),color=beePop)) + geom_line()
  ...
  finalsum <- ddply(finalstate,.(beePop,simnum), function(x){return(pct(x))})
  
  finalsum <- finalstate %>% group_by(beePop,simnum) %>% dplyr::summarise(
    meanVisits=mean(cumulativeVisits),
    percentVisit = length(cumulativeVisits[cumulativeVisits>0]/length(cumulativeVisits)))
  
  
    percentsvisits <- ggplot(finalsum,aes(x=beePop,y=percentVisit, color=simnum)) +
    geom_point(size=3,position=position_jitter(width=.2))+
    scale_y_continuous("Percent of flowers not visited") +
    scale_x_discrete("Bees per 100 flowers")
  
  percentsvisits
  
  
  learning <- simulations[[1]]$learndata
  for (i in 2:length(simulations)){
    row<-simulations[[i]]$learndata
    learning<-rbind(learning,row)
  }
  
  learningPlot <- ggplot(learning,aes(x=TrueVal,y=Threshold))
  learningPlot + geom_point(aes(color=beePop)) +geom_abline(aes(intercept=0,slope=1))
  ...
  simulation2 <- function(nbees,signalDelt,sugarDelt) { # we could name some parameters we wanted to study in this line and then specify them in the initation calls!
    ...
    # ggplot(ti,aes(x=signal,y=sugar))+geom_point()+geom_smooth(method="lm")
    # 
    Microbe <- MicrobeInit(sugarDelt = sugarDelt,signalDelt = signalDelt ,growthRate = 20)
    ...
    for(i in 1:200){
      ...
      test <- mapply(FUN=function(x,y){
        tmp <- daytimestep(flower=x,bee=y,microbe=Microbe)
      },Field,encounterList,SIMPLIFY=F)
  ...
  Parms <- data.frame(sug=rep(Parms$Var1,5),sig=rep(Parms$Var2,10))
  ParmsList <- split(Parms,seq(nrow(Parms))) #need to feed mclapply a list
  
 
  microbeSensitivity <- mclapply(X=ParmsList,FUN =  function(x){simulation2(nbees=1,signalDelt = x$sig, sugarDelt = x$sug)},mc.cores = 11) 
 
 
  sensBees <- do.call(rbind,microbeSensitivity[[1]]$bees)
  sensBees$sugDelt <- Parms[1,"sug"]
  sensBees$sigDelt <- Parms[1,"sig"]
  sensBees$simNum <- 1
  for (i in 2:nrow(Parms)){
    d <-do.call(rbind,microbeSensitivity[[i]]$bee)
    d$sugDelt <- Parms[i,"sug"]
  ...
  sensFlowers$sugDelt <- Parms[1,"sug"]
  ...
  SensEnd <- sensFlowers[sensFlowers$time==max(sensFlowers$time),]
  
  pct2<-function(x){length(x[x>0]} 
 
  SumSensEnd <-  
    SensEnd %>% 
    group_by(sugDelt,sigDelt,simNum) %>% 
    dplyr::summarize(cumulative=mean(cumulativeVisits),
                     microbeP = mean(microbesP1),
                     pct = length(cumulativeVisits[cumulativeVisits>0])/length(cumulativeVisits) )
  
  
  
  
  ggplot(SumSensEnd, aes(x=sigDelt,y=pct,))+geom_point()+facet_grid(~sugDelt)
